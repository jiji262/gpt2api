...
<template>
  <div class="page-container">
    <PageHeader crumb="管理员 / 设置" title="系统设置" accent-word="设置" accent="pink">
      <template #extra>
        <NeonButton variant="outline" size="sm" :icon="Refresh" @click="doReload">强制重载</NeonButton>
        <NeonButton variant="outline" size="sm" :icon="MailIcon" @click="mailDlg = true">测试邮件</NeonButton>
        <NeonButton
          variant="ghost"
          size="sm"
    ...
    <el-dialog v-model="mailDlg" title="发送 SMTP 测试邮件" width="420px">
      ...
      <template #footer>
        <NeonButton variant="ghost" @click="mailDlg = false">取消</NeonButton>
        <NeonButton variant="pink" :loading="mailSending" @click="submitTestMail">发送</NeonButton>
      </template>
    </el-dialog>
  </div>
</template>
