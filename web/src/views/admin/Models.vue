...
<template>
  <div class="page-container">
    <PageHeader crumb="管理员 / 模型" title="模型配置" accent-word="配置" accent="pink">
      <template #extra>
        <NeonButton v-if="canWrite" variant="pink" size="sm" @click="openCreate">
    ...
    <el-table v-loading="loading" :data="filteredRows" stripe>
      ...
      <el-table-column prop="slug" label="对外 slug" min-width="170">
        <template #default="{ row }">
          <span class="num-mono">{{ row.slug }}</span>
        </template>
      </el-table-column>
      <el-table-column label="类型" width="90">
        <template #default="{ row }">
          <StatusTag :variant="row.type === 'chat' ? 'cyan' : 'yellow'">
      ...
      <el-table-column prop="upstream_model_slug" label="上游 slug" min-width="170" show-overflow-tooltip>
        <template #default="{ row }">
          <span class="num-mono">{{ row.upstream_model_slug }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="状态" width="90">
        <template #default="{ row }">
          ...
          />
          <StatusTag v-else :variant="row.enabled ? 'active' : 'disabled'" dot>
            {{ row.enabled ? '启用' : '禁用' }}
          </StatusTag>
        </template>
      </el-table-column>
    ...
    >
      ...
      <template #footer>
        <NeonButton variant="ghost" @click="dlgVisible = false">取消</NeonButton>
        <NeonButton variant="pink" :disabled="dlgLoading" @click="submit">保存</NeonButton>
      </template>
    </el-dialog>
  </div>
</template>
