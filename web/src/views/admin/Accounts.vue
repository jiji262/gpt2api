...
// ========== 状态/类型展示 ==========
const statusMap: Record<string, { label: string; variant: 'active' | 'warning' | 'info' | 'danger' | 'disabled' }> = {
  healthy:    { label: '健康', variant: 'active'   },
  warned:     { label: '风险', variant: 'warning'  },
  throttled:  { label: '限流', variant: 'warning'  },
  suspicious: { label: '可疑', variant: 'info'     },
  dead:       { label: '失效', variant: 'danger'   },
}
function statusVariant(s: string) { return statusMap[s]?.variant || 'disabled' }
...
const formDefault = {
  ...
  auto_delete_conversation: false,
  notes: '',
  cookies: '',
  proxy_id: 0,
  status: 'healthy',
...
async function openEdit(row: accountApi.Account) {
  ...
  Object.assign(form, {
    ...
    auto_delete_conversation: !!row.auto_delete_conversation,
    notes: row.notes || '',
    cookies: '',
    proxy_id: 0,
  ...
  try {
    if (!isEdit.value) {
      ...
      const body: any = {
        ...
        auto_delete_conversation: form.auto_delete_conversation,
...
<template>
  <div class="page-container">
    <PageHeader crumb="管理员 / 账号" title="GPT 账号" accent-word="账号" accent="cyan">
      <template #extra>
        <NeonButton variant="outline" :loading="batchRunning === 'probe'" :disabled="loading" @click="onProbeAll">
          全部探测
        </NeonButton>
        <NeonButton variant="outline" :loading="batchRunning === 'refresh'" :disabled="loading" @click="onRefreshAll">
          全部刷新
        </NeonButton>
        <el-dropdown trigger="click" @command="onBulkDelete">
          <NeonButton variant="outline">批量删除</NeonButton>
          <template #dropdown>
            <el-dropdown-menu>
        ...
        <NeonButton variant="cyan" @click="openCreate">新建账号</NeonButton>
      </template>
    </PageHeader>
    ...
    <div class="filter-bar">
      ...
      <NeonButton variant="cyan" size="sm" @click="onSearch">搜索</NeonButton>
    ...
    >
      <el-table-column label="邮箱" min-width="200" show-overflow-tooltip>
        <template #default="{ row }">
          <el-tooltip v-if="row.notes" placement="top" :content="row.notes">
      ...
      <el-table-column label="状态" width="90">
        <template #default="{ row }">
          <StatusTag :variant="statusVariant(row.status)" dot>{{ statusText(row.status) }}</StatusTag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="最近刷新" min-width="148" show-overflow-tooltip>
        <template #default="{ row }">
          ...
          <el-tooltip v-if="row.refresh_error" placement="top" :content="row.refresh_error">
      ...
      <el-table-column label="操作" width="240" fixed="right">
        <template #default="{ row }">
          <div class="op">
            <a class="op-link" :class="{ loading: refreshingIds.has(row.id) }"
               @click="!refreshingIds.has(row.id) && onRefreshOne(row)">刷新</a>
            <a class="op-link" :class="{ loading: probingIds.has(row.id) }"
               @click="!probingIds.has(row.id) && onProbeOne(row)">探测</a>
            <a class="op-link" @click="openBind(row)">代理</a>
    ...
    <el-dialog v-model="dlg" :title="isEdit ? '编辑账号' : '新建账号'" width="720px" destroy-on-close>
      <el-form label-width="120px" size="default">
        ...
        <el-form-item label="生图后删对话">
          <el-checkbox v-model="form.auto_delete_conversation">
            成功后自动在 chatgpt.com 侧删除该对话
          </el-checkbox>
          <div style="font-size:12px; color:#909399; margin-top:4px; line-height:1.5">
            等价于用户在网页点"删除"按钮(PATCH is_visible=false),避免 chatgpt.com
            左侧列表堆积"IMG2 xxx"记录。服务端记录仍保留,不影响 24h 内图片再下载。
          </div>
        </el-form-item>
      ...
      <template #footer>
        ...
        <NeonButton variant="cyan" :loading="submitting" @click="submitForm">确定</NeonButton>
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="bindDlg" title="绑定代理" width="420px">
      ...
      <template #footer>
        <NeonButton variant="ghost" @click="bindDlg = false">取消</NeonButton>
        <NeonButton variant="cyan" @click="submitBind">确定</NeonButton>
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="importDlg" title="批量导入账号" width="760px" destroy-on-close>
      ...
      <template #footer>
        <NeonButton variant="ghost" :disabled="importing" @click="importDlg = false">关闭</NeonButton>
        <NeonButton variant="cyan" :loading="importing" @click="doImport">
          开始导入
...
.drop-zone {
  ...
  .drop-ic { font-size: 30px; color: $c-cyan-text; margin-bottom: 6px; }
  .drop-text {
    font-size: 14px; color: $n-ink;
    .link { color: $c-cyan-text; cursor: pointer; text-decoration: underline; margin-left: 2px; }
  }
  .drop-sub { margin-top: 6px; font-size: 12px; color: $gray-500; }
}
...
.file-list {
  ...
  .file-list-head {
    ...
    padding-bottom: 4px; color: $gray-500; font-size: 12px;
    border-bottom: 1px dashed $gray-200; margin-bottom: 4px;
  }
  .file-row {
    display: flex; align-items: center; justify-content: space-between;
    padding: 4px 0; font-size: 13px;
    .fname { flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .fsize { color: $gray-500; margin: 0 8px; font-variant-numeric: tabular-nums; }
  }
}

.progress {
  margin-top: 16px; padding: 12px 14px;
  background: var(--el-fill-color-lighter); border-radius: $r-sm;
  .progress-head {
    display: flex; justify-content: space-between; align-items: center;
    margin-bottom: 6px;
...
.err-list {
  margin-top: 12px;
  .err-list-head { color: var(--el-color-danger); font-weight: 500; margin-bottom: 6px; }
}
</style>
