...
// buildTextMessage 构造 text 通路的一条消息。
//
// 带附件时 content 从 text 变 multimodal_text,parts 前面插 asset_pointer,
// 并在 metadata 里补 attachments —— 形状与 image 通路完全一致(见 image.go),
// 差别只在 text 通路保留 selected_sources、不写 system_hints。
//
// ⚠ 这个组合(text 通路 + attachments)**没有 HAR 抓包实证**。
// 它按两条已验证通路的交集推导而来,理论上正确,但上游对
// "客户端类型"的判定非常敏感,形状不对会触发 silent rejection。
// 因此 vision 默认关闭,由 gateway.vision_enabled 显式开启,见 docs/spec/vision.md。
func buildTextMessage(m ChatMessage) map[string]interface{} {
	...
	if atts := validAttachments(m.Attachments); len(atts) > 0 {
		parts := make([]interface{}, 0, len(atts)+1)
		list := make([]Attachment, 0, len(atts))
		for _, u := range atts {
			parts = append(parts, u.ToAssetPointerPart())
			list = append(list, u.ToAttachment())
		}
...
// validAttachments 过滤掉上传失败的占位项。
func validAttachments(in []*UploadedFile) []*UploadedFile {
	out := make([]*UploadedFile, 0, len(in))
	for _, u := range in {
		if u != nil && u.FileID != "" {
			out = append(out, u)
		}
...
func (c *Client) StreamFChat(ctx context.Context, opt FChatOpts) (<-chan SSEEvent, error) {
	...
	go parseSSE(ctx, res.Body, out, opt.SSETimeout)
	return out, nil
}
