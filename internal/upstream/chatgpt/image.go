...
// HideConversation 等价于用户在 chatgpt.com 网页点"删除对话"(垃圾桶)时发的
// PATCH /backend-api/conversation/{id}  body {"is_visible": false}。
//
// 服务端记录仍保留,attachment / file 下载端点继续工作——这是故意的,让本项目
// 的 24h 代理 URL(`/p/img/<task_id>/<idx>`)在 sediment 图上仍能再取签名 URL。
//
// 失败返回 UpstreamError;调用方应视为 best-effort(对话留在侧栏不影响主流程)。
func (c *Client) HideConversation(ctx context.Context, convID string) error {
	if convID == "" {
		return fmt.Errorf("conv_id required")
	}
	body := strings.NewReader(`{"is_visible":false}`)
	req, err := http.NewRequestWithContext(ctx, http.MethodPatch,
		c.opts.BaseURL+"/backend-api/conversation/"+url.PathEscape(convID), body)
	if err != nil {
		return err
	}
	c.commonHeaders(req)
	...
	buf, _ := io.ReadAll(res.Body)
	if res.StatusCode >= 400 {
		return &UpstreamError{Status: res.StatusCode, Message: "hide conversation failed", Body: string(buf)}
	}
	return nil
}
