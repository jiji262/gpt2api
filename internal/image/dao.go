...
// MarkLocalStored 标记 N 张图的字节已经全部落盘到本地目录,记录 content-type。
// 调用时机:Runner 在生图 SSE/poll 成功 → 遍历 SignedURLs 下载字节 → 每张都写
// 磁盘成功后,再一次性调用(全部成功才标记,任一失败则保持 local_stored=0)。
func (d *DAO) MarkLocalStored(ctx context.Context, taskID, contentType string) error {
	_, err := d.db.ExecContext(ctx, `
UPDATE image_tasks
   SET local_stored = 1, local_content_type = ?
 WHERE task_id = ?`, truncate(contentType, 64), taskID)
	return err
}
...
SELECT id, task_id, user_id, key_id, model_id, account_id, prompt, n, size, upscale, status,
       conversation_id, file_ids, result_urls, local_stored, local_content_type,
