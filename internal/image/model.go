...
type Task struct {
	...
	// LocalStored=true 时,N 张图的字节已落盘到
	// {ImageLocalRoot}/{task_id}/{idx}.{ext};images_proxy 直接读本地文件,
	// 不再回源 chatgpt。LocalContentType 记录实际 MIME(多为 image/png)。
	LocalStored      bool       `db:"local_stored"`
	LocalContentType string     `db:"local_content_type"`
