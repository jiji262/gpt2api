...
func (r *Runner) runOnce(ctx context.Context, opt RunOptions, result *RunResult) (bool, string, error) {
	...
	// 7) 立刻把 N 张图的字节下载到本地 ./data/images/{task_id}/{idx}.{ext}。
	// 这一步成功后,images_proxy 读本地文件不再回源 chatgpt —— 哪怕 conversation
	// 被 hide/删,哪怕 sediment 签名过期,前端仍能正常展示。
	//
	// 任一张下载/落盘失败就不打 local_stored 标记,保持 result_urls 回源兜底。
	// opt.TaskID 为空的同步直出模式跳过(无 task_id 就没落盘路径也没法写 DB)。
	localOK := false
	var savedCT string
	if opt.TaskID != "" && len(signedURLs) > 0 {
		allOK := true
		for i, u := range signedURLs {
			fetchCtx, cancel := context.WithTimeout(ctx, 45*time.Second)
			data, ct, err := cli.FetchImage(fetchCtx, u, 16*1024*1024)
			cancel()
			if err != nil {
				logger.L().Warn("image runner fetch for local store failed",
					zap.String("task_id", opt.TaskID),
					zap.Int("idx", i),
					zap.Error(err))
				allOK = false
			...
			if _, err := SaveImage(DefaultLocalRoot, opt.TaskID, i, ct, data); err != nil {
				logger.L().Warn("image runner save local failed",
					zap.String("task_id", opt.TaskID),
					zap.Int("idx", i),
					zap.Error(err))
				allOK = false
		...
		if allOK {
			localOK = true
			if r.dao != nil {
				if err := r.dao.MarkLocalStored(ctx, opt.TaskID, savedCT); err != nil {
					logger.L().Warn("image runner mark local_stored failed (file ok,dB miss)",
						zap.String("task_id", opt.TaskID), zap.Error(err))
				}
			}
			logger.L().Info("image runner local stored",
				zap.String("task_id", opt.TaskID),
				zap.Uint64("account_id", lease.Account.ID),
				zap.Int("count", len(signedURLs)),
				zap.String("content_type", savedCT))
		}
	}

	// 8) 账号勾选了"生图后删对话",**且**这次图片字节已完整落盘到本地,
	// 才对 chatgpt 发 PATCH is_visible=false(等价于网页点"删除")。
	// 落盘未完成时绝不 hide —— 否则 sediment attachment 端点会 404,
	// 前端永远拿不到图。fire-and-forget,失败只 warn。
	if localOK && lease.Account.AutoDeleteConversation && convID != "" {
		go func(cid string, accountID uint64) {
			hideCtx, cancel := context.WithTimeout(context.Background(), 8*time.Second)
			defer cancel()
			if err := cli.HideConversation(hideCtx, cid); err != nil {
				logger.L().Warn("image runner hide conversation failed",
					zap.Uint64("account_id", accountID),
					zap.String("conv_id", cid),
					zap.Error(err))
				return
			}
			logger.L().Info("image runner hide conversation ok",
				zap.Uint64("account_id", accountID),
				zap.String("conv_id", cid))
		}(convID, lease.Account.ID)
	}
