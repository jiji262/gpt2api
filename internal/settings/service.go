...
// SiteAPIBaseURL 是对外可访问的 /v1 入口根地址(不含尾斜杠)。
// 留空时由请求头推导,见 gateway.publicBaseURL。
func (s *Service) SiteAPIBaseURL() string {
	return strings.TrimRight(s.GetString(SiteAPIBaseURL), "/")
}
...
// GatewayVisionEnabled 控制 chat 的图片输入。默认关闭:
// text 通路 + attachments 的组合没有 HAR 抓包实证,见 docs/spec/vision.md。
func (s *Service) GatewayVisionEnabled() bool { return s.GetBool(GatewayVisionEnabled) }
