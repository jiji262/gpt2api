...
// AutoDeleteConversation 用指针区分"未传"和"显式设为 false"。
...
func (s *Service) Create(ctx context.Context, in CreateInput) (*Account, error) {
	...
	a := &Account{
		...
		AutoDeleteConversation: in.AutoDeleteConversation,
...
func (s *Service) Update(ctx context.Context, id uint64, in UpdateInput) (*Account, error) {
	...
	if in.AutoDeleteConversation != nil {
		a.AutoDeleteConversation = *in.AutoDeleteConversation
	}
	if in.Status != "" {
		a.Status = in.Status
	}
