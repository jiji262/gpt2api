...
import (
	...
	"github.com/jiji262/gpt2api/internal/account"
	"github.com/jiji262/gpt2api/internal/apikey"
	"github.com/jiji262/gpt2api/internal/audit"
	"github.com/jiji262/gpt2api/internal/auth"
	"github.com/jiji262/gpt2api/internal/backup"
	...
	"github.com/jiji262/gpt2api/internal/middleware"
	"github.com/jiji262/gpt2api/internal/model"
	"github.com/jiji262/gpt2api/internal/proxy"
	"github.com/jiji262/gpt2api/internal/rbac"
	"github.com/jiji262/gpt2api/internal/recharge"
	"github.com/jiji262/gpt2api/internal/settings"
	"github.com/jiji262/gpt2api/internal/usage"
	"github.com/jiji262/gpt2api/internal/user"
	pkgjwt "github.com/jiji262/gpt2api/pkg/jwt"
...
func New(d *Deps) *gin.Engine {
	...
	v1 := r.Group("/v1", apikey.Middleware(d.KeySvc, false))
	{
		...
		// 官方有单模型检索端点,不少 SDK / 中转层会用它探测模型可用性。
		v1.GET("/models/:model", d.GatewayH.RetrieveModel)
		v1.POST("/chat/completions", d.GatewayH.ChatCompletions)
		// Responses 无状态转译垫片。有状态子端点(按 id 回查/取消/列举)显式 501。
		v1.POST("/responses", d.GatewayH.Responses)
		v1.GET("/responses/:id", d.GatewayH.ResponsesUnsupported)
		v1.DELETE("/responses/:id", d.GatewayH.ResponsesUnsupported)
		v1.POST("/responses/:id/cancel", d.GatewayH.ResponsesUnsupported)
		v1.GET("/responses/:id/input_items", d.GatewayH.ResponsesUnsupported)
	...
	// NoRoute 无条件注册:纯 API 部署下也要保证 /v1 的未知路径回结构化错误。
	r.NoRoute(notFoundHandler(mountSPA(r)))
